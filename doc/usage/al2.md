...
2. [bootstrap.sh](https://github.com/aws-samples/amazon-eks-ami-rhel/blob/main/src/runtime/bootstrap.sh) does modify a few fields, like `kubeReserved` and `evictionHard`, so you'd need to modify the config after the bootstrap script is run and restart `kubelet` to overwrite those properties.

...
case "${(L)SUBNET_OPTION}" in
  public|private|all)
    ...
    for subnet in ${=subnets}; do
      ...
      [ -n "$cleaned_subnet" ] && {
        $(aws ec2 describe-subnets --subnet-ids $cleaned_subnet --query "Subnets[0].MapPublicIpOnLaunch" --output text) = "True" && \
        public_subnets+="${cleaned_subnet} " || private_subnets+="${cleaned_subnet} "
      }
    done
